...
///
/// # Generics
/// - `C`: Custom part of the [`Context`] that is passed to custom endpoints.
#[derive(Debug, Deserialize, Serialize)]
pub struct Context<C = ()> {
    // "Static" properties
    /// Some name for the server.
    #[serde(skip)]
    pub name: String,

    // User-provided properties
    /// The address where the server will be bound.
    pub addr: SocketAddr,
    ...
    // Custom context
    /// The custom `C`ontext defined by the user.
    pub custom: C,
}
